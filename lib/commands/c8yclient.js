...
function preprocessResponseObject(r) {
...
function run(client, fns, prevSubject, options, baseUrl) {
  ...
  return getNextPact().then(async (pact) => {
    // console.log("--------------------------------");
    // console.log(JSON.stringify(pact, undefined, 2));
    const response = await new Cypress.Promise((resolve, reject) => {
      const resolver = async (response) => {
        ...
        if (_.isArray(response)) {
          result = response.map((r) => preprocessResponseObject(r));
        } else {
          result = preprocessResponseObject(response);
        }
