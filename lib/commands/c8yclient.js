  ...
  return cy.then(async () => {
    ...
    if (!Cypress.c8ypact.isRecordingEnabled()) {
      for (const r of _.isArray(response) ? response : [response]) {
        getNextPact().then((pact) => {
          if (!pact && Cypress.c8ypact.failOnMissingPacts) {
            throwError(`Pact ${Cypress.c8ypact.currentPactIdentifier()} not found and Cypress.c8ypact.failOnMissingPacts is enabled.`);
          }
          if (pact) {
