...
function preprocessResponseObject(r, client, save = true) {
  ...
  if (client.options) {
    response.options = client.options;
  }

  if (client.auth) {
    response.auth = _.pick(client.auth, ["user", "userAlias"]);
    ...
    response.auth.type = client.auth.constructor.name;
  }
...
function authenticateClient(auth, baseUrl) {
  return cy.then(async () => {
    ...
    preprocessResponseObject(res, clientCore, false);
    if (res.status !== 200) {
...
function run(client, fns, prevSubject, options, baseUrl) {
  ...
  return cy.then(async () => {
    const matchPact = (response) => {
      if (
        Cypress.c8ypact.isEnabled() &&
        !Cypress.c8ypact.isRecordingEnabled()
      ) {
        for (const r of _.isArray(response) ? response : [response]) {
          const optionsPact = options.pact;
          (optionsPact
            ? cy.wrap(optionsPact, { log: false })
            : Cypress.c8ypact.currentNextPact()
          ).then((pact) => {
            const ignore =
              options.ignorePact ||
              (Cypress.config().c8ypact && Cypress.config().c8ypact.ignore) ||
              false;
            if (pact && !ignore) {
              cy.c8ymatch(r, pact, options);
            } else {
              if (!pact && Cypress.c8ypact.failOnMissingPacts && !ignore) {
                throwError(
    ...
    const response = await new Cypress.Promise(async (resolve, reject) => {
      ...
      const preprocessedResponse = async (promise) => {
        ...
        result = preprocessResponseObject(result, client);
...
Cypress.Commands.add("c8ymatch", (response, pact, options = {}) => {
