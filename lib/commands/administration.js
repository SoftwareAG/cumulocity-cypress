...
Cypress.Commands.add('clearUserRoles', { prevSubject: 'optional' }, (...args) => {
  ...
  return cy
    .wrap(auth, { log: false })
    .c8yclient((client) => client.user.detail(userIdentifier))
    .then((response) => {
      ...
      cy.wrap(assignedRoles, { log: false }).each((assingedRole) => {
        cy.wrap(auth, { log: false }).c8yclient((client) =>
          client.userGroup.removeUserFromGroup(assingedRole.group.id, userIdentifier)
        );
      });
      ...
      [
        ...
        (client, groupResponse) =>
          client.userGroup.addUserToGroup(
            ...
            `https://${client.core.baseUrl}/user/${client.core.tenant}/users/${userIdentifier}`
          ),
      ],
      clientOptions
    );
