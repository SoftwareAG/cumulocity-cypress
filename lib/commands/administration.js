...
Cypress.Commands.add('createUser', { prevSubject: 'optional' }, function (...args) {
      ...
      if (permissions && !_.isEmpty(permissions)) {
        cy.wrap(permissions, { log: false }).each((permission) => {
          cy.wrap(auth, { log: false }).c8yclient(
            [
              (c) => c.core.fetch('/user/' + c.core.tenant + '/groupByName/' + permission),
              (c, groupResponse) =>
                c.userGroup.addUserToGroup(groupResponse.body.id, userResponse.body.self),
            ],
            clientOptions
          );
        });
          ...
          cy.wrap(auth, { log: false }).c8yclient(
            [
              ...
              (c, applicationResponse) => {
                ...
                const applications =
                  applicationResponse.body.applications || applicationResponse.body;
                if (_.isArray(applications)) {
                  const apps = applications.map((a) => {
                    if (_.isString(a)) {
                      ...
                      return _.pick(_.defaults(a, { type: 'HOSTED' }), ['id', 'type']);
                    }
                    return undefined;
                  });
...
Cypress.Commands.add('deleteUser', { prevSubject: 'optional' }, function (...args) {
  const $args = normalizedArgumentsWithAuth(args);
  ...
  return cy
    .wrap(auth, { log: false })
    .c8yclient((c) => c.user.delete(_.isObjectLike(user) ? user.userName : user), options)
    .then((deleteResponse) => {
  ...
  return cy
    ...
    .then((response) => {
      ...
      cy.wrap(assignedRoles, { log: false }).each((assingedRole) => {
        cy.wrap(auth, { log: false }).c8yclient(
          (client) => client.userGroup.removeUserFromGroup(assingedRole.group.id, userIdentifier),
          options
        );
      });
    ...
    cy.wrap(auth, { log: false }).c8yclient(
      [
        ...
        (client, groupResponse) => {
          const baseUrl = options && !!options.baseUrl ? options.baseUrl : client.core.baseUrl;

          return client.userGroup.addUserToGroup(
            groupResponse.body.id,
            `https://${baseUrl}/user/${client.core.tenant}/users/${userIdentifier}`
          );
        },
      ],
      options
...
Cypress.Commands.add('getCurrentTenant', { prevSubject: 'optional' }, function (...args) {
  const $args = normalizedArgumentsWithAuth(args);
...
Cypress.Commands.add('getTenantId', { prevSubject: 'optional' }, function (...args) {
  const $args = normalizedArgumentsWithAuth(args);
  const [auth] = $args;
  const consoleProps = {
    auth,
...
Cypress.Commands.add('getSystemVersion', { prevSubject: 'optional' }, function (...args) {
  const $args = normalizedArgumentsWithAuth(args);
  const [auth, clientOptions] = $args;
...
Cypress.Commands.add('bootstrapDeviceCredentials', { prevSubject: 'optional' }, function (...args) {
  const $args = normalizedArgumentsWithAuth(args);
  ...
  return cy
    ...
    .then((response) => {
      ...
      let result;
      if (response.status === success && response.body && response.body.username) {
